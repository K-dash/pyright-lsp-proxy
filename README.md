...
> **Frozen capabilities on cold start** — `initialize` always answers immediately with empty capabilities, on every startup, regardless of whether a `.venv` exists yet — the first backend, like every other, is created lazily on the first file open ([#140](https://github.com/K-dash/typemux-cc/issues/140): a synchronous pre-spawned-backend handshake here used to permanently wedge Claude Code 2.1.207's LSP client). That advertisement stays frozen for the session — verified with Claude Code 2.1.207 that the client keeps sending LSP requests regardless, so nothing is gated (see [Frozen Empty Capabilities](docs/ARCHITECTURE.md#frozen-empty-capabilities-important)).
